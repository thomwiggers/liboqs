 ...
 */
int PREFIX_NAME(crypto_sign_keypair)(uint8_t *pk, uint8_t *sk)
{
...
int PREFIX_NAME(crypto_sign)(
  ...
  const uint8_t *sk)
{
    *smlen=mlen+PREFIX_NAME(CRYPTO_BYTES);
    memcpy(sm+PREFIX_NAME(CRYPTO_BYTES),m,mlen);
...
{
    int result;
    *mlen=smlen-PREFIX_NAME(CRYPTO_BYTES);
    result=sign_openHFE(sm+PREFIX_NAME(CRYPTO_BYTES),(size_t)(*mlen),sm,pk);
    /* For compatibily with SUPERCOP, the memcpy is done only after sign_open */
    memcpy(m,sm+PREFIX_NAME(CRYPTO_BYTES),(size_t)(*mlen));
    return result;
}
...
int PQCLEAN_GEMSS128_AVX2_crypto_sign_signature(
    ...
    *siglen = PREFIX_NAME(CRYPTO_BYTES);

    return signHFE(sig, m, mlen, (UINT*)sk);
}
...
int PQCLEAN_GEMSS128_AVX2_crypto_sign_verify(
    ...
    (void)siglen;
    return sign_openHFE(m, mlen, sig, pk);
}
