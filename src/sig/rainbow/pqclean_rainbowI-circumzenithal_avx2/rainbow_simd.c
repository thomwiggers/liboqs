...
int PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_rainbow_sign( uint8_t * signature , const sk_t * sk , const uint8_t * _digest )
{
    ...
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_hash_msg( prng_seed , _HASH_LEN , prng_preseed , _HASH_LEN+LEN_SKSEED );
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_set( &prng_sign , prng_seed , _HASH_LEN );                                   // seed = H( sk_seed || digest )
    ...
    while( !l1_succ ) {
        ...
        PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_gen( &prng_sign , vinegar , _V1_BYTE );                       // generating vinegars
    ...
    while( !succ ) {
        ...
        PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_gen( &prng_sign , salt , _SALT_BYTE );                        // roll the salt
        PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_hash_msg( _z , _PUB_M_BYTE , digest_salt , _HASH_LEN+_SALT_BYTE ); // H(digest||salt)
        ...
        PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256v_set_zero( temp_o , _O2_BYTE );
...
int _rainbow_verify( const uint8_t * digest , const uint8_t * salt , const unsigned char * digest_ck )
{
    ...
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_hash_msg( correct , _PUB_M_BYTE , digest_salt , _HASH_LEN+_SALT_BYTE );  // H( digest || salt )
...
int PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_rainbow_verify( const uint8_t * digest , const uint8_t * signature , const pk_t * pk )
{
    unsigned char digest_ck[_PUB_M_BYTE];
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_rainbow_publicmap( digest_ck , pk->pk , signature );
...
int PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_rainbow_sign_cyclic( uint8_t * signature , const csk_t * csk , const uint8_t * digest )
{
    sk_t _sk;
    sk_t * sk = &_sk;
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_generate_secretkey_cyclic( sk, csk->pk_seed , csk->sk_seed );   // generating classic secret key.

    int r = PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_rainbow_sign( signature , sk , digest );

    memset( sk , 0 , sizeof(sk_t) ); // clean
    return r;
}

int PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_rainbow_verify_cyclic( const uint8_t * digest , const uint8_t * signature , const cpk_t * pk )
{
    unsigned char digest_ck[_PUB_M_BYTE];
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_rainbow_publicmap_cpk( digest_ck , pk , signature );
