...
void PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_calculate_Q_from_F_simd( ext_cpk_t * Qs, const sk_t * Fs , const sk_t * Ts )
{
/// Layer 1
/*
...
void PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_calculate_F_from_Q_simd( sk_t * Fs , const sk_t * Qs , const sk_t * Ts )
{
    // the size might be large.
#if defined(_MALLOC_)
    unsigned char * t1 = (unsigned char *) adapted_alloc( 32 , _V1*_O1*32 );
...
void PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_calculate_Q_from_F_cyclic_simd( cpk_t * Qs, const sk_t * Fs , const sk_t * Ts )
{
/// Layer 1 , Q5, Q3, Q6, Q9
/*
    Q_pk.l1_F5s[i] = UT( T1tr* (F1 * T1 + F2) )
...
#define _SIZE_BUFFER_F3 (_O2_BYTE * _V1 * _O2)
    ...
    memset( Qs->l1_Q5 , 0 , _O1_BYTE * N_TRIANGLE_TERMS(_O1) );
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_UpperTrianglize( Qs->l1_Q5 , buffer_F3 , _O1, _O1_BYTE );    /// UT( ... )   /// Q5
/*
    F1_T2     = F1 * t2
    F2_T3     = F2 * t3
    F1_F1T_T2 + F2_T3 = F1_T2 + F2_T3 + F1tr * t2
...
*/
    ...
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_UpperTrianglize( Qs->l1_Q9 , buffer_F3 , _O2 , _O1_BYTE );   /// Q9
