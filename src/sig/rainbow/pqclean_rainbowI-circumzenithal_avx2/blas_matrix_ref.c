...
void PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf16mat_prod_ref(uint8_t *c, const uint8_t *matA, unsigned n_A_vec_byte, unsigned n_A_width, const uint8_t *b) {
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256v_set_zero(c, n_A_vec_byte);
...
void PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256mat_prod_ref(uint8_t *c, const uint8_t *matA, unsigned n_A_vec_byte, unsigned n_A_width, const uint8_t *b) {
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256v_set_zero(c, n_A_vec_byte);
    for (unsigned i = 0; i < n_A_width; i++) {
...
unsigned PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf16mat_solve_linear_eq_32x32_ref(uint8_t *sol, const uint8_t *inp_mat, const uint8_t *c_terms ) {
...
unsigned PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf16mat_inv_32x32_ref(uint8_t *inv_a, const uint8_t *a ) {
    const unsigned H=32;
    uint8_t mat[32*32];
    for (unsigned i = 0; i < H; i++) {
        uint8_t *ai = mat + i * 32;
        PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256v_set_zero(ai, 32 );
...
unsigned PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256mat_solve_linear_eq_48x48_ref( uint8_t * sol , const uint8_t * inp_mat , const uint8_t * c_terms )
{
    const unsigned n = 48;
    ...
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256v_set_zero(mat,n*vec_len); // clean
    return r8;
}
...
unsigned PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256mat_inv_32x32_ref( uint8_t * inv_a , const uint8_t * a )
{
    const unsigned H=32;
    uint8_t mat[H*H*2];
    for(unsigned i=0;i<H;i++) {
        uint8_t * ai = mat + i*2*H;
        PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256v_set_zero( ai , 2*H );
        gf256v_add( ai , a + i*H , H );
        ai[H+i] = 1;
    }
    unsigned char r8 = gf256mat_gauss_elim_ref( mat , H , 2*H );
    gf256mat_submat( inv_a , H , H , mat , 2*H , H );
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256v_set_zero(mat,H*2*H);
    return r8;
}
...
unsigned PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256mat_solve_linear_eq_64x64_ref( uint8_t * sol , const uint8_t * inp_mat , const uint8_t * c_terms )
{
    const unsigned n = 64;
    const unsigned vec_len = n + _BLAS_UNIT_LEN_;
...
unsigned PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256mat_inv_36x36_ref( uint8_t * inv_a , const uint8_t * a )
{
    const unsigned H=36;
    uint8_t mat[H*H*2];
    for(unsigned i=0;i<H;i++) {
