...
void madd_reduce( unsigned char * y , const unsigned char * tmp_res , unsigned vec_len )
{
    ...
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256v_set_zero( y , vec_len );
    // x1
    for(int i=1;i<_GFSIZE;i+=2) gf256v_add( y , tmp_res+TMPVEC_LEN*i , vec_len );
    // x2
    accu_bit = 1<<1; // 2
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_gf256v_set_zero( tmp , vec_len );
...
void PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_rainbow_publicmap( unsigned char * y, const unsigned char * trimat, const unsigned char * x )
{
...
void PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_rainbow_publicmap_cpk( unsigned char * z, const cpk_t * pk, const unsigned char *w )
{
    prng_t prng0;
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_set( &prng0 , pk->pk_seed , LEN_PKSEED );
...
error: buffer size.
    ...
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_gen( &prng0 , buffer , sizeof(_sk->l1_F1) ); // l1_F1
    accu_eval_quad( tmp , buffer , _v1 , _V1 , _O1_BYTE );

    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_gen( &prng0 , buffer ,  sizeof(_sk->l1_F2) );  // l1_F2
    ...
    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_gen( &prng0 , buffer ,  sizeof(_sk->l2_F1) ); // l2_F1
    accu_eval_quad( tmp2 , buffer , _v1 , _V1 , _O2_BYTE );

    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_gen( &prng0 , buffer ,  sizeof(_sk->l2_F2) ); // l2_F2
    accu_eval_quad_rect( tmp2 , _v1 , _V1 , buffer , _o1 , _O1 , _O2_BYTE );

    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_gen( &prng0 , buffer ,  sizeof(_sk->l2_F3) ); // l2_F3
    accu_eval_quad_rect( tmp2 , _v1 , _V1 , buffer , _o2 , _O2 , _O2_BYTE );

    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_gen( &prng0 , buffer ,  sizeof(_sk->l2_F5) ); // l2_F5
    accu_eval_quad( tmp2 , buffer , _o1 , _O1 , _O2_BYTE );

    PQCLEAN_RAINBOWICIRCUMZENITHAL_AVX2_prng_gen( &prng0 , buffer ,  sizeof(_sk->l2_F6) ); // l2_F6
